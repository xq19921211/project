...
function* initLoginLogRecordList(action) {
  try {
    const req = {
      method: 'GET',
      credentials: 'include',
      ...
      url: urlGenerator(
        window.hempConfig.apiPath + '/log/login/list',
        action.payload,
      ),
    };
    // let param;
    ...
    //     param = item + '=' + '&'
    // }
    // const url = urlGenerator(window.hempConfig.basePath + '/log/login/list', action.payload);
...
export default function* loginLogRecordSaga() {
  yield takeEvery(LOG_RECORD, initLoginLogRecordList);
}
